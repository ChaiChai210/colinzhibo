...
public class TCControllerFullScreen extends RelativeLayout implements IController, View.OnClickListener,
        TCVodMoreView.Callback, TCVodQualityView.Callback, SeekBar.OnSeekBarChangeListener {

    // UI控件
    private RelativeLayout mLayoutTop;                             // 顶部标题栏布局
    private ImageButton mIvBack;                                // 顶部标题栏中的返回按钮
    ...
    //中间
    private ImageButton mIvLock;                                // 锁屏按钮
    ...
    private AppCompatSeekBar mSeekBarProgress;                       // 播放进度条
    private TextView mTvQuality;                             // 当前画质文本
    private ImageButton mIvFullScreen;
...
//                    setThumbnail(progress);
    ...
    private void initView(Context context) {
        ...
        mIvDanmu = findViewById(R.id.iv_danmuku);
...
//                    int y = (int) (height * mWaterMarkBmpY) - bmp.getHeight() / 2;
                    ...
                    mIvWatermark.setX(mWaterMarkBmpX);
                    mIvWatermark.setY(mWaterMarkBmpX);
...
//        mSeekBarProgress.setPointList(pointParams);
        ...
        if (!mLockScreen) {
            if (event.getAction() == MotionEvent.ACTION_UP && mVideoGestureUtil != null && mVideoGestureUtil.isVideoProgressModel()) {
                ...
                if (mPlayType == SuperPlayerConst.PLAYTYPE_LIVE || mPlayType == SuperPlayerConst.PLAYTYPE_LIVE_SHIFT) {
                    ...
                    Log.e("chia", "mDuration" + mDuration + "percentage" + percentage);
                }
                if (mControllerCallback != null) {
    ...
    public void onClick(View view) {
        int i = view.getId();
        if (i == R.id.iv_back || i == R.id.tv_title ||i == R.id.iv_fullscreen) { //顶部标题栏
            if (mControllerCallback != null) {
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean isFromUser) {
        if (mGestureVideoProgressLayout != null && isFromUser) {
            ...
            mGestureVideoProgressLayout.setProgress(progress);
//            Log.e("chia", "onProgressChanged" + progress);
        }
        // 加载点播缩略图
