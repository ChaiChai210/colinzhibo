...
public class MayLikeAdapter extends BaseQuickAdapter<VideoGuessLikeEntity, BaseViewHolder> {

    private final SpannableStringBuilder mBuilder;

    public MayLikeAdapter(@Nullable List<VideoGuessLikeEntity> data) {
        super(R.layout.item_details_like, data);
        mBuilder = new SpannableStringBuilder();
    }

    @Override
    protected void convert(@NonNull BaseViewHolder baseViewHolder, @Nullable VideoGuessLikeEntity VideoGuessLikeEntity) {
        ImageView imageView = baseViewHolder.getView(R.id.iv_avatar);
//        Glide.with(getContext()).load( VideoGuessLikeEntity.getIntroduction_image()).placeholder(R.mipmap.ic_detail_like_ph).centerCrop().into(imageView);
        ImageUtils.displayImage(getContext(), VideoGuessLikeEntity.getThumb(), R.mipmap.ic_detail_like_ph, imageView);
        String numFormat = getContext().getResources().getString(R.string.hint_play_num);
        baseViewHolder.setText(R.id.tv_num, getNumFormatString(mBuilder, VideoGuessLikeEntity.getPlay_count(), numFormat));
        baseViewHolder.setText(R.id.tv_title, VideoGuessLikeEntity.getTitle());
        if (!TextUtils.isEmpty(VideoGuessLikeEntity.getTag())) {
            LabelsView labelsView = baseViewHolder.getView(R.id.labels);
            labelsView.setLabels(ArrayUtils.asArrayList(VideoGuessLikeEntity.getTags().split("\\|")));
        }
        baseViewHolder.itemView.setOnClickListener(v -> ARouter.getInstance().build(ARConstants.PATH_VIDEO_PLAY).withInt("id", VideoGuessLikeEntity.getId()).navigation());
